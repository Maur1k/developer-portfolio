...
export default function About() {
  ...
  return (
    <section className="min-h-screen py-24 px-4 md:px-8 relative">
      <div className="absolute inset-0 overflow-hidden pointer-events-none"><div className="absolute top-40 right-0 w-96 h-96 bg-gradient-to-bl from-orange-500/5 to-amber-400/5 rounded-full blur-3xl"></div></div>
      <div className="relative z-10 max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="mb-16">
          ...
          <h2 className="text-5xl md:text-6xl font-bold text-white mb-4">Building practical software with modern tools.</h2>
        </motion.div>
        ...
        <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid lg:grid-cols-2 gap-12 items-start">
          <motion.div variants={itemVariants} className="space-y-6">
            <div><h3 className="text-2xl font-bold text-white mb-4">Who I Am</h3><p className="text-gray-400 text-lg leading-relaxed whitespace-pre-line">{profile.aboutMe}</p></div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="space-y-6">
            {education.length > 0 && (
              <div className="group relative">
                ...
                <div className="relative glass-card group-hover:border-orange-500/40 rounded-2xl p-8 transition-all duration-300">
                  ...
                  <div className="space-y-5">
                    {education.map((entry) => (
                      <div key={entry.id} className="border-b border-white/5 pb-4 last:border-b-0 last:pb-0">
                        <p className="text-amber-400 font-semibold">{entry.degree}</p>
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="mt-2 flex flex-wrap gap-3 text-sm">
                            {certificate.credentialUrl && (
                              <a href={certificate.credentialUrl} target="_blank" rel="noreferrer" className="text-amber-400 hover:text-white transition-colors">View Credential</a>
                            )}
                            {certificate.pdfUrl && (
                              <a href={certificate.pdfUrl} target="_blank" rel="noreferrer" className="text-amber-400 hover:text-white transition-colors">Download PDF</a>
                            )}
                          </div>
                        </div>
                      </div>
